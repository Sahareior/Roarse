...
const RecentShipment = ({
  ...
  const getMethodIcon = () => {
    switch (method.toLowerCase()) {
      ...
      case "sea":
        return <FaShip className="text-green-500" />;
      default:
    ...
    <div className="flex justify-between items-center p-4 border border-[#E5E7EB] rounded-lg hover:bg-gray-50 transition-colors duration-200">
      <div className="flex-1">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span
            className={`${statusInfo.color} py-1 px-3 rounded-full text-sm font-medium`}
          >
            {statusInfo.text}
          </span>
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 my-3">
                <p className="px-4 py-1 bg-[#333333] rounded-2xl text-sm">
                  Premium
                </p>
                <p className="px-4 py-1 bg-[#00A63E] rounded-2xl text-sm">
                  Verified
                </p>
              </div>
            </div>
...
const ViewProfile = () => {
  ...
  // Mock documents with image URLs (use placeholders or actual paths)
  const documents = [
    {
      title: "Business License",
      imageUrl:
        "https://fastly.picsum.photos/id/111/536/354.jpg?hmac=fHcbrbPdjjAKJDWHzXpGbfQ9Jbxs3x-FEQNm_rQrmzQ",
    },
    {
  ...
  const handleVerify = () => {
    // Implement verify logic here (e.g., API call, state update)
    alert(`Verifying ${selectedDocument?.title}...`);
    // For example, you could update a verified state if needed
    handleCloseModal();
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border rounded-xl p-5">
          ...
          <ul className="space-y-7 text-sm">
            ...
            <li className="flex border-b-2 border-[#767575] pb-3 justify-between">
              <span className="text-[16px] text-[#767575] flex items-center gap-2">
                <IoLocationSharp /> Saved Address
              </span>
              <span className="font-medium text-[24px]">8</span>
            </li>
            <li className="flex border-b-2 border-[#767575] pb-3 justify-between">
              <span className="text-[16px] text-[#767575] flex items-center gap-2">
                <FaStar /> Preferred Carrier
              </span>
              <span className="font-medium text-[24px]">8</span>
            </li>
          </ul>
        ...
        <div className="space-y-4">
          {mockShipments.length > 0 ? (
            mockShipments.map((shipment) => (
              <RecentShipment key={shipment.id} {...shipment} />
            ))
          ) : (
            <div className="text-center py-8 border border-[#E5E7EB] rounded-lg">
              <p className="text-gray-500">
                No shipments found with the selected filter.
              </p>
            </div>
          )}
        </div>
